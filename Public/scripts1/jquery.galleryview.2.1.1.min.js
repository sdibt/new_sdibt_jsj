...
(function($){$.fn.galleryView=function(g){var h=$.extend($.fn.galleryView.defaults,g);
var j;var k=0;var l=0;var m;var n;var o=false;
var q;var r;var t;var u;var v;var w;var z;var A;
var B;var C=20;var D;var E;var F;var G={};
var H={};var I={};var J={};
var K=true;var L=false;var M;var N;var O;var P;var Q;var R;
function showItem(i){$('.nav-next-overlay',M).unbind('click');
$('.nav-prev-overlay',M).unbind('click');
$('.nav-next',M).unbind('click');
$('.nav-prev',M).unbind('click');
O.unbind('click');
if(h.show_filmstrip){
	O.removeClass('current').find('img').stop().animate({'opacity':h.frame_opacity},h.transition_speed);
	O.eq(i).addClass('current').find('img').stop().animate({'opacity':1.0},h.transition_speed)}
...
if(h.show_filmstrip){if(m=='strip'){
	N.stop();var b;var c;
	if(F=='horizontal'){
		b=getPos(O[i]).left-(getPos(R[0]).left+(u/2)-(A/2));
		c=(b>=0?'-=':'+=')+Math.abs(b)+'px';
		N.animate({'left':c},h.transition_speed,h.easing,function(){
			var a=i;if(i>l){
				i=i%l;k=i;N.css('left','-'+((A+h.frame_gap)*i)+'px')}
			else if(i<=(l-strip_size)){i=(i%l)+l;k=i;
			N.css('left','-'+((A+h.frame_gap)*i)+'px')}
			if(a!=i){
				O.eq(a).removeClass('current').find('img').css({'opacity':h.frame_opacity});
				O.eq(i).addClass('current').find('img').css({'opacity':1.0})}
			if(!h.fade_panels){
				Q.hide().eq(i%l).show()}$('.nav-prev-overlay',M).click(showPrevItem);
				$('.nav-next-overlay',M).click(showNextItem);
				$('.nav-prev',M).click(showPrevItem);
				$('.nav-next',M).click(showNextItem);enableFrameClicking()})}
	else{
		b=getPos(O[i]).top-(getPos(R[0]).top+(t)-(B/2));c=(b>=0?'-=':'+=')+Math.abs(b)+'px';
		N.animate({'top':c},h.transition_speed,h.easing,function(){
			var a=i;if(i>l){i=i%l;k=i;N.css('top','-'+((B+h.frame_gap)*i)+'px')}
			else if(i<=(l-strip_size)){i=(i%l)+l;k=i;N.css('top','-'+((B+h.frame_gap)*i)+'px')}
			if(a!=i){O.eq(a).removeClass('current').find('img').css({'opacity':h.frame_opacity});O.eq(i).addClass('current').find('img').css({'opacity':1.0})}
			if(!h.fade_panels){
				Q.hide().eq(i%l).show()}$('.nav-prev-overlay',M).click(showPrevItem);
				$('.nav-next-overlay',M).click(showNextItem);
				$('.nav-prev',M).click(showPrevItem);
				$('.nav-next',M).click(showNextItem);enableFrameClicking()})}}
else if(m=='pointer'){R.stop();var d=getPos(O[i]);
if(F=='horizontal'){
	R.animate({'left':(d.left+(A/2)-(u/2)+'px')},h.transition_speed,h.easing,function(){
		if(!h.fade_panels){
			Q.hide().eq(i%l).show()}$('.nav-prev-overlay',M).click(showPrevItem);
			$('.nav-next-overlay',M).click(showNextItem);
			$('.nav-prev',M).click(showPrevItem);
	...
	R.animate({'top':(d.top+(B/2)-(t)+'px')},h.transition_speed,h.easing,function(){if(!h.fade_panels){Q.hide().eq(i%l).show()}$('.nav-prev-overlay',M).click(showPrevItem);
	$('.nav-next-overlay',M).click(showNextItem);$('.nav-prev',M).click(showPrevItem);$('.nav-next',M).click(showNextItem);enableFrameClicking()})}}}};function extraWidth(a){
		if(!a){
			return 0}
		if(a.length==0){
			return 0}a=a.eq(0);
			var b=0;b+=getInt(a.css('paddingLeft'));
			b+=getInt(a.css('paddingRight'));
			b+=getInt(a.css('borderLeftWidth'));
			b+=getInt(a.css('borderRightWidth'));return b};
			function extraHeight(a){
				if(!a){return 0}if(a.length==0){
					return 0}
				a=a.eq(0);var b=0;
				b+=getInt(a.css('paddingTop'));
				b+=getInt(a.css('paddingBottom'));
				b+=getInt(a.css('borderTopWidth'));
				b+=getInt(a.css('borderBottomWidth'));return b};
				function showNextItem(){$(document).stopTime("transition");
				if(++k==O.length){
					k=0}showItem(k);
					if(!o){$(document).everyTime(h.transition_interval,"transition",function(){showNextItem()})}};
					function showPrevItem(){$(document).stopTime("transition");
					if(--k<0){k=l-1}showItem(k);
					if(!o){$(document).everyTime(h.transition_interval,"transition",function(){
						showNextItem()})}};
						function getPos(a){
							var b=0,top=0;var c=a.id;
							if(a.offsetParent){
								do{
									b+=a.offsetLeft;
									top+=a.offsetTop}while(a=a.offsetParent)}if(c==j){
										return{'left':b,'top':top}}
									else{
										var d=getPos(M[0]);
										var e=d.left;
										var f=d.top;
										return{'left':b-e,'top':top-f}}};
										function enableFrameClicking(){O.each(function(i){if($('a',this).length==0){$(this).click(function(){if(k!=i){$(document).stopTime("transition");
										showItem(i);
										k=i;if(!o){
